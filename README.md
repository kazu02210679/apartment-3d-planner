...
- 編集中にキャンバスへフォーカスがある状態で `End` を押すと、カタログの優先設置面（なければ許可された床）へ最寄りの有効位置に補正します。`Shift+End` は部屋の床へ補正します。右クリックの補正メニューは `範囲内に戻す`、`最寄りの支持面に置く`、`床に置く` の最大 3 操作です。ロック、非表示、入力欄、メニュー、ダイアログ、IME 中は実行しません。
